...
module.exports = class {
  ...
  toEnvVarName(key) {
    ...
    // key1 => KEY_1
    return _.upperCase(key).replace(/\s/g, '_')
  }
  ...
  traverseUp(key) {
    ...
    const path = key.substr(0, index)
    key = key.substr(index + 1)

    const envVar = this.toEnvVarName(path)
    const envValue = process.env[envVar]
    if (envValue && (envValue.startsWith('{') || envValue.startsWith('['))) {
      const json = JSON.parse(envValue)
      const value = dot.pick(key, json)
      if (value !== undefined) return value
    }
    return this.traverseUp(path)
  }

  getValueFromEnv(key) {
    const envVarName = this.toEnvVarName(key)

    if (!process.env.hasOwnProperty(envVarName)) return this.traverseUp(key) || undefined

    return process.env[envVarName] || this.traverseUp(key)
  }
